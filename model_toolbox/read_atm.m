function [P,T,RH,EX,error_msg] = read_atm(atmfile,BOX_WALL,Times);
%function [P,T,RH,EX,error_msg] = read_atm(atmfile);
% read P, T, RH values from netcdf file
 ...
 % MVCM edit: There is no longer one extra time in the master time level file
 ...
 varid = netcdf.inqVarID(ncid,'EXCH_COEFF');
 EX(:,:) = netcdf.getVar(ncid,varid);
 netcdf.close(ncid)
