...
% read in master time and level file
 ...
 % initialize model time steps
   ...
   disp('dt_chem sec');
   disp(dt_chem);
   disp('dt_diff_horiz sec');
   disp(dt_diff_horiz);
   disp('NTIM_INTEGRATION');
   disp(NTIM_INTEGRATION);
   disp('NTIM_CHEM:');
   disp(NTIM_CHEM);
  ...
  % initialize the temperature and pressure, currently just numbers, fix
   ...
   % MVCM 10/22: added a space for exchange coefficient on initialization
   excoeff = zeros(NLEV,NTIM_CHEM);
   excoeff(:,:)  = 0;
   ...
   % return exchange coefficient when reading atmfile 10/22, MVCM
   [pressure_input,temperature_input,relative_humidity_input,ex_coeff_input,err_msg] = read_atm(atmfile,BOX_WALL,Times);
   if err_msg
       clean_exit('initialize_model.m',err_msg)
   end
  
   %interpolate T, P, RH in time ot the chemistry integration times
   % MVCM added horizontal exchange coefficient 10/22
   for l=1:NLEV
   ...
   VT(:,:,1)                = 0.;
   HT                       = zeros(NVAR,NLEV,NTIM_CHEM);                  %vertical transport rates
   HT(:,:,1)                = 0.;
 ...
 % read jrate file
   ...
   %interplate jrates in time, to the chemistry timesteps
    ...
    %read backgroun column file
    bkgfile                           = [model_path '/input/bkg_col.nc'];
    [bkg_in, bkg_list, err_msg]    = read_bkg(bkgfile,BOX_WALL,Times);
    if err_msg
        clean_exit('initialize_model.m',err_msg)
    end
   
   %set up space for the bkg profile
   [Nbkg, ~, ~] = size(bkg_in);
   spec_bkg = zeros(Nbkg,NLEV,NTIM_CHEM);
   spec_bkg(:,:,:) = fill_value_netcdf;
   %interpolate bkg in time, to the chemistry timesteps
   for i=1:Nbkg
       for l=1:NLEV   
           spec_bkg(i+1,l,:)    = interp1(squeeze(datenum_input),squeeze(bkg_in(i,l,:)),squeeze(datenum_chem),'linear');
       end
   end
  ...
  % MVCM 10/22 added background filepath
  bkgfile                              = [model_path '/input/bkg_col.nc'];
  temperature_t0                       = temperature(:,1);
  pressure_t0                          = pressure(:,1);
  relative_humidity_t0                 = relative_humidity(:,1);
  % MVCM 10/22 added exchange coefficient
  excoeff_t0                 = excoeff(:,1);
  % MVCM 10/22 changed init_spec to read initial background column concentrations
  [spec,spec_fixed,err_msg]            = init_spec(initial_spec_file, NLEV, spec, spec_fixed, Times, BOX_WALL, temperature_t0, pressure_t0, relative_humidity_t0);    
  %[spec,spec_fixed,spec_bkg,err_msg]            = init_spec(initial_spec_file, bkgfile, NLEV, spec, spec_fixed, spec_bkg, Times, BOX_WALL, temperature_t0, pressure_t0, relative_humidity_t0); 
  %set species names and spec cocnetration at NTIM=1
...
%      emission_rates_file                  = [model_path '/input/Emission_rates_new.nc'];
