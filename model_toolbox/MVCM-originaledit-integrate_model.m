% edited by MVCM 03/2023: added horizontal transport rates

function [spec, spec_fixed, rates, VT, HT, dt_diff_horiz, rate_constants] = integrate_model(...
    model_path,...
    NTIM_CHEM,...
    NLEV,...
    dt_chem,...
...
for t=1:NTIM_CHEM-1   %since we integrate to t = t+1, loop to NTIM_CHEM-1
    ...
    if (run_vert_diff == 1) %if doing the vertical diffusion, do this
        ...
        temp_HT=zeros(NVAR,NLEV); %temp array for horizontal transport, this is summed
        ...
        for counter=1:1:diff_rep
            ...
            % MVCM 05/2023: fixing factor of two error in Kz calculation
            %Kz_interp = Kz1+Kz2;
            Kz_interp = (Kz1+Kz2)/2;
            
            % MVCM 05/2023: fixing factor of two error in diffusion constant calculation
            %diff_const_interp = diff_const1+diff_const2;
            diff_const_interp = (diff_const1+diff_const2)/2;

            % MVCM 03/2023: only do horizontal mixing when counter / counter_div has a remainder of 0
            counter_div = (dt_diff_horiz * n_step_diff)/dt_chem;
            m = rem(counter, counter_div);
            ...
            [spec(:,:,t+1), temp_VT(:,:), temp_HT(:,:), temp_depo(:,1), total_loss_to_ground(:,t+1)] = diffusion_1d (spec(:,:,t+1), spec_bkg(:,:,t+1), BOX_WALL, BOXCH, ...           
                Kz_interp, excoeff(:,t+1), diff_const_interp, ...
                rho, temperature, NLEV, dt_diff, dt_diff_horiz, Eff_dep_surf(:,t),  n_step_diff, total_loss_to_ground(:,t+1), run_chem, add_surface_source_HONO, horizontal_mix);

            %sum the deposition to the total deposition
            depo(:,t+1)=depo(:,t+1)+temp_depo(:,1);
            VT(:,:,t+1)=VT(:,:,t+1)+temp_VT(:,:);
            HT(:,:,t+1)=HT(:,:,t+1)+temp_HT(:,:);
        ...
        % MVCM 03/2023: set number of horizontal advection timestamps by dividing the chemistry timestep in seconds by the horizontal timestep in seconds
        n_ts_horiz = dt_chem / dt_diff_horiz;
        HT(:,:,t+1)=HT(:,:,t+1)/double(n_ts_horiz);
        ...
        VT(:,:,t+1)=0;
        HT(:,:,t+1)=0;
        disp(['-->Skipping 1D diffusion and horizontal advection']);
    end
    ...
    if((WriteCounter == WriteRep) || (t == NTIM_CHEM-1 ) )
        ...
        disp('');
        disp('');
        write_netcdf_output(model_path, NLEV, t, spec, spec_fixed, temperature, press, rh, rates,rate_constants,VT,HT,depo,addemissions,surface_source_emissions,...
