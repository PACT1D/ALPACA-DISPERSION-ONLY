% MVCM 03/23: added file for horizontal transport rates as calculated in diffusion_1d.m
function [  ] = open_vertical_transport_file( model_path, Times, nlev, ntim, DateStrLen, VT, HT, BOX_WALL, BOXCH, output_file_comment, output_file_created_by )
...
% MVCM 03/23 added:
%----------------------- write horizontal transport rates to netcdf file --------------------
%write netcdf file with the horizontal transport rates
ncid = netcdf.create([model_path '/output/ht_rates.nc'], 'CLOBBER');
...
varid_box = netcdf.defVar(ncid,'BOX_WALL','double',[z_dimid]);
netcdf.putAtt(ncid,varid_box,'units','m')
netcdf.putAtt(ncid,varid_box,'long_name','horizontal grid spacing, upper boundary of model grid box in meters')

for j = 1:NVAR
  varid(j)  = netcdf.defVar(ncid,spec_names{j},'double', [z_dimid, time_dimid]); 
  netcdf.putAtt(ncid,varid(j),'units','molec/cm3/s')
  netcdf.putAtt(ncid,varid(j),'long_name',['horizontal transport rate for species - ' spec_names{j}])
  netcdf.putAtt(ncid,varid(j),'_FillValue',fill_value_netcdf)
end

varid_boxch = netcdf.defVar(ncid,'BOXCH','double',[z_dimid]);
netcdf.putAtt(ncid,varid_boxch,'units','m')
netcdf.putAtt(ncid,varid_boxch,'long_name','horizontal grid spacing, center point of model grid box in meters')
...
%add add horizontal transport rates
for j=1:NVAR
  ind = get_ind(spec_names{j});
  tmp = squeeze(HT(ind,:,1));
  netcdf.putVar(ncid,varid(j),[0,0],[nlev,1],tmp);
end
