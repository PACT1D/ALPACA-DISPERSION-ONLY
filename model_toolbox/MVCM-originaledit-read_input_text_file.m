...
%set model options, timesteps, and comments
   ...
   for i=1:22
       ...
       %reactions, etc
       % MVCM 03/23: extended from i<20 to i<21 to account for new line in model_setup.txt
       if(i>3 && i<21)
           %output = textscan(line, '%d', 'CollectOutput', 1);
           % MVCM 04/2023 changed "%d" to "%f" to be able to read values with decimals
           output = textscan(line, '%f', 'CollectOutput', 1);
       ...
       % MVCM 03/23: changed from i==20 and 1==21 to 1==21 and i==22 to account for new line in model_setup.txt
       if(i==21)
           comment1 = line;
       end
       if(i==22) 
 ...
 % global value for including surface loss in model level 1 - yes=1 or no=0
   ...
   % MVCM 03/23: added time difference between horizontal advection timestamps
   global dt_diff_horiz;
   dt_diff_horiz=options_in(17); 
   output_file_comment = comment1;
   output_file_created_by = comment2;
   
   %write this back to output file
   % MVCM 03/23: also writes out horizontal advection timestep when running
   ...
   fprintf(fileID,'%s  \t %d\n','horizontal advection dt_diff',dt_diff_horiz);
