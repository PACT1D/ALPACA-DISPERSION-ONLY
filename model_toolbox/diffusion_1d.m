...
function [ spec_t1 , VT_t , HT_t , depo_t, total_loss_to_ground] = diffusion_1d (spec_t0, spec_bkg, BOX_WALL, BOXCH, ...
    ...
    dt_diff_in, Eff_dep_vel_t,  n_step_diff, total_loss_to_ground, run_chem, add_surface_source_HONO)
...
%for ii = 1:N_TIME_DIFF
    ...
    for nn = 1:NVAR
        ...
        % MVCM 05/2023: added horizontal advection process below to occur at the same timing as vertical diffusion
        ...
        for k=1:NLEV
            ...
            frac_bkg_step = exch_coeff_interp(k) * double(dt_diff);
        ...
        spec_after_Ht(nn,:) = spec_t1(nn,:);
        % convert species back to concentration
        for k=1:NLEV
            spec_after_Vt(nn,k) = spec_after_Vt(nn,k) * rho(k); 
            spec_after_Ht(nn,k) = spec_after_Ht(nn,k) * rho(k);
            spec_t1(nn,k) = spec_t1(nn,k) * rho(k);
        end
...
HT_t = (spec_after_Ht-spec_after_Vt)/double(dt_diff_in);
